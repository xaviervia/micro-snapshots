...
const runTest = (snapshotsDirPath: string, overwrite: boolean) => ([
  name,
  test,
]: Test) => {
...
export const runFiles = (
  filePaths: string[],
  {
    ...
    shouldOverwrite,
    shouldMatch,
  }: { snapshotsFolderName?: string, shouldOverwrite?: boolean, shouldMatch?: string}
) => {
  filePaths.forEach((filePath) => {
    const resolvedFilePath = path.resolve(filePath)
    const { tests }: { tests: Test[] } = require(resolvedFilePath)

    run(tests, resolvedFilePath, snapshotsFolderName ?? '__snapshots__', shouldOverwrite ?? false, shouldMatch)
  })
}
